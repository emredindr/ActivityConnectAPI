...
using ActivityConnect.Business.ValidationRules.FluentValidation.User;
...
public class UserAppService : BaseAppService, IUserAppService
{
    ...
    [ValidationAspect(typeof(UserRegisterInputValidator))]
    public async Task<UserLoginOutput> Register(UserRegisterInput input)
    {
        var userCheck = await _userRepository.GetAsync(x => x.Email == input.Email || x.UserName == input
        .UserName);
        if (userCheck != null)
            throw new ApiException($"{input.Email} yada {input.UserName} 'e ait kullanıcı zaten mevcut.");

        var createdUserId = await _userRepository.InsertAndGetIdAsync(new User
        {
            Name = input.Name,
        ...
        var newUserCheck = await _userRepository.GetAsync(x => x.Id == createdUserId);

        if (newUserCheck == null)
            throw new ApiException($"{createdUserId} id numaralı kayıt bulunamadı");

        var userPermissionList = await _userPermissionAppService.GetUserPermissions(createdUserId);

        var accessToken = _jwtAuthenticationManager.CreateToken(newUserCheck, userPermissionList);

        var mappedItem = Mapper.Map<UserLoginOutput>(newUserCheck);
